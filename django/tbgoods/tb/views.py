...
from tb.models import info,UserInfo,pl,jd_info,jd_pl,bj,job_info
import csv
...
def pl_page(request):      #数据分页
    pl_list = pl.objects.with_counts()  # 自定义管理器查询
...
def download_job_info(request):  #--------------------------------------下载数据库数据为csv
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename=job_info_list.csv'
        writer = csv.writer(response)
        writer.writerow(['s_name','time','name','ares','money','company','people','fuli','describes','url'])  #定义字段名
        job_infos=job_info.objects.all()          #获取所需数据
        for job_infos in job_infos:    #遍历数据列表
           writer.writerow([job_infos.s_name,job_infos.time,job_infos.name,job_infos.ares,job_infos.money,job_infos.company,job_infos.people,job_infos.fuli,job_infos.describes,job_infos.url])  #获取模型所需的字段
        return response


def search_job_info(request):       #搜索数据
    q = request.GET.get('q') #对应htmlinput当中name属性 --  #request.GET.get('q') 获取到用户提交的搜索关键词。用户通过表单提交的数据 django 为我们保存在 request.GET 里，这是一个类似于 Python 字典的对象，所以我们使用 get 方法从字典里取出键 q 对应的值，即用户的搜索关键词。这里字典的键之所以叫 q 是因为我们的表单中搜索框 input 的 name 属性的值是 q，如果修改了 name 属性的值，那么这个键的名称也要相应修改。
    error_msg = ''
    if not q:
        error_msg = '请输入关键词'
        return render(request, 'tb/job_info_list_s.html', {'error_msg': error_msg})
    job_info_list = job_info.objects.filter(s_name__icontains=q)   #用户输入了搜索关键词，我们就通过 filter 方法从数据库里过滤出符合条件的所有数据。icontains为模糊匹配，相当于sql的like
    return render(request, 'tb/job_info_list_s.html', {'error_msg': error_msg, 'job_info_list':job_info_list})


def job_info_page(request):      #数据分页
    job_info_list = job_info.objects.all()  # 获取所有info,假设在models.py中已定义了info模型
    paginator = Paginator(job_info_list, 20) # 每页25条

    page = request.GET.get('page')
    try:
        job_infos = paginator.page(page) # infos为Page对象！返回在提供的下标处的 Page  对象，下标以1开始。如果提供的页码不存在，抛出 InvalidPage  异常。
    except PageNotAnInteger:
        # If page is not an integer, deliver first page.
        job_infos = paginator.page(1)
    except EmptyPage:
        # If page is out of range (e.g. 9999), deliver last page of results.
        job_infos = paginator.page(paginator.num_pages)   #页面总数。

    return render(request, 'tb/job_info_list_page.html', {'job_infos': job_infos})   #render()这是一个非常常见的习惯用语，用于加载模板，填充上下文并返回一个含有模板渲染结果的 HttpResponse 对象
